...
public class ServerConnection {
	...
	private static Hashtable<String,Message> msgList=new Hashtable<String,Message>() {{
		...
		put("dwstaterequest", new StateRequest() );
	
		put("dwmaxrequest", new MaxRequest() );	
		put("userdwstatevalueon", new UserStateValue("on") );
		put("userdwstatevalueoff", new UserStateValue("off") );
		put("userdwstatevaluepause", new StatePause() );

		
		put("userdwmaxvalue",  new UserMaxValue());
		put("rtrequest",  new RemainingTimeRequest());
		put("userdwrtvalue",  new UserRemainingTimeValue());
	...
	public ServerConnection(String configFile)
	{
		...
		//verify file reading
		System.out.println("");
		System.out.println("**************** Dishwasher entity ******************");
		System.out.println("");
		...
		this.sendToBuildingManager("DishwasherConnected");
...
msgList.get(msge.toLowerCase()).handle(val);}
	...
	public String receiveNewMessage()
	{
		...
		for (int i=newPositionVect; i < messages.size(); i++)
		{
			...
			//System.out.println("Display => msg " + i + " => " + messages.elementAt(i));
